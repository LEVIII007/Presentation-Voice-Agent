...
def _line(*, event, session, processor, ttfb_ms, ts):
    return json.dumps(
        {
            "record": {
                "extra": {
    ...
    log_path.write_text(
        "\n".join(
            [
                ...
                _line(event="ttfb", session="sess-a", processor="AzureLLMService#0", ttfb_ms=2400.0, ts=1001.0),
                _line(event="ttfb", session="sess-a", processor="CartesiaTTSService#0", ttfb_ms=140.0, ts=1001.4),
                _line(event="turn", session="sess-a", processor="ignored", ttfb_ms=0, ts=1002.0),
                _line(event="ttfb", session="sess-b", processor="AzureLLMService#1", ttfb_ms=1800.0, ts=2000.0),
            ]
        ),
        encoding="utf-8",
    )
